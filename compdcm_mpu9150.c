...
volatile uint8_t g_bleGapFlag;
volatile uint8_t g_bleBondFlag;
...
main(void)
{
    ...
    while(1)
    {
        ...
        if(g_ui32PrintSkipCounter >= PRINT_SKIP_COUNT)
        {
			...
			if (g_bleGapFlag == 1 && g_bleBondFlag == 1) {
				g_bleFlag = 0;
				ble_cmd_attributes_write(20, 0, 4, (uint8_t*)&pfData[0]);
				while (g_bleFlag == 0) {
				}
				g_bleFlag = 0;
				ble_cmd_attributes_write(24, 0, 4, (uint8_t*)&pfData[1]);
				while (g_bleFlag == 0) {
				}
				g_bleFlag = 0;
				ble_cmd_attributes_write(28, 0, 4, (uint8_t*)&pfData[2]);
				while (g_bleFlag == 0) {
				}
			}
